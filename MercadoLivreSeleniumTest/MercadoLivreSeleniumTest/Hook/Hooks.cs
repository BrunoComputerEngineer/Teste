...
namespace MercadoLivreSeleniumTest.Hook
{
    ...
    public class Hooks
    {
        ...
        public void AfterScenario()
        {
            ...
            string tagScenario = ScenarioContext.Current.ScenarioInfo.Tags[0];
            if (tagScenario != "TestMethods" && tagScenario != "API" && tagScenario != "APIToken")
            {
                try
                {
                    //loginInteractions.SetLogout_Click();
                    //loginInteractions.BtnLogout_Click();
                }
                catch
                {
